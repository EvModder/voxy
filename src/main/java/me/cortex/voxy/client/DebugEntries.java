...
public class DebugEntries {
    ...
    public static void onRebuild(Map<Identifier, DebugScreenEntryStatus> allStatuses) {
        var entry = allStatuses.getOrDefault(GPU_DEBUG, DebugScreenEntryStatus.NEVER);
        boolean enabled = entry != DebugScreenEntryStatus.NEVER;
        if (enabled != previousGpuDebugEnabled) {
            previousGpuDebugEnabled = enabled;

            boolean vulkan = MinecraftVkHost.isMinecraftOnVulkan();
            GPUTiming.INSTANCE.setEnabled(enabled && !vulkan);
            RenderStatistics.enabled = enabled && !vulkan;
            if (!vulkan) {
    ...
    public static boolean isGpuDebugEnabled() {
        return previousGpuDebugEnabled;
    }
}
