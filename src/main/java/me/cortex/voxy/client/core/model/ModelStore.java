...
public class ModelStore implements IModelStore {
    ...
    @Override
    public void uploadModelTexture(int modelId, me.cortex.voxy.common.util.MemoryBuffer texture, boolean hasMips) {
        final int TS = ModelFactory.MODEL_TEXTURE_SIZE;
        int X = (modelId&0xFF) * TS*3;
        int Y = ((modelId>>8)&0xFF) * TS*2;
        long cAddr = texture.address;
        for (int lvl = 0; lvl < (hasMips ? ModelFactory.LAYERS : 1); lvl++) {
